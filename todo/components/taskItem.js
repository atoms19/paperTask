...
import { tasks, sound } from "../oldapp.js";
...
export default  function TaskItem(task) {
   return el("li")
      ...
      .attr(task.done ? "checked" : "unchecked", "")
      .checkFor("input", (e) => {
         task.done = true;
      ...
      .addChild(
         priorityTagItem(task.priority).style({
            textDecoration: task.done ? "line-through" : "none"
         })
      )
      .$end()
      ._el("button", { ariaLabel: "close", title: "remove task" })
      .html(
      ...
      .onClick((s) => {
         if (task.note.trim() != "" || task.streak > 0) {
            let ans = confirm(
               "this task has a note inside it ?,mind opening and looking before deleting"
            );
            if (ans == true) {
               MoreInfoItem(task);
               return;
