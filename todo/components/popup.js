...
export default function MoreInfoItem(task) {
   ...
   el("dialog", { open: "" })
      ...
      ._el("header")
      ._el("button", { ariaLabel: "close", rel: "prev" })
      .onClick((s) => {
         s.parent().parent().parent().remove();
      })
      .$end()
      ._el("p")
      ._el("strong", `📝 ${task.name}`)
      .$end()
      ._el("p", "🔥streak: " + (task.streak || "0") + " days")
      .style({
         fontSize: "0.8rem",
         marginLeft: "1.35rem",
         marginTop: "0.4rem",
      ...
      .attr("data-tooltip", "will be updated on 12am")
      .showIf(habit)
      .$end()
      .$end()
      .$end()
      ...
      ._el("select", { name: "select", ariaLabel: "select priority" })
      .loops(priorityTAGs, (p, s, i) => {
         s._el("option", p).$end();
      })
      .model(priority)
      .$end()
      ...
      .$end()
      ._el("footer")
      ._el("span", `created on: ${task.createdOn}`)
      .style({
