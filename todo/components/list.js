import { tasks, lists } from "../oldapp.js";
...
export default function List(tasks, listName) {
   ...
   return el("details", { class: "list" })
      ._el("summary", listName)
      .enableHold(2)
      .checkFor("hold", () => {
      ...
      .$end()
      .showIf(tasks, () => listBox.value.length > 0);
}
...
function showListEditor(listBox, listName) {
   ...
   return el("dialog", { open: "" })
      ...
      ._el("h3", "list actions for " + listName)
      .$end()
      ._el("div", { role: "group" })
      ._el("button", "spread list to today")
      .onClick(() => {
         tasks.set(
            tasks.value.map((v) => {
      ...
      .enableHold(2)
      .checkFor("hold", (e) => {
         defaultSpread = prompt(
            "you can enter now spread to any list , enter the list to be spread to"
         );
      ...
      .onClick(async () => {
         ...
         let shareUrl =
            ...
            "?q=" +
            encodeURI(JSON.stringify([listName, toBeShared]));
         if (navigator.share) {
            try {
            ...
            copy(shareUrl);
            alert(
               "share url has been copied to your clipboard ,share it by any sms or text means"
            );
         }
      })
      .$end()
